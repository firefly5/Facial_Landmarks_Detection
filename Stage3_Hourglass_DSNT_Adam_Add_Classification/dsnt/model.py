...
class HumanPoseModel(nn.Module):
    ...
    def _hm_preact(self, x, preact):
        ...
        # do softmax every row, totally C rows
        if preact == 'softmax':
...
class ResNetHumanPoseModel(HumanPoseModel):
    ...
    def __init__(self, resnet, n_chans=21, dilate=0, truncate=0, output_strat='dsnt',
...
class HourglassHumanPoseModel(HumanPoseModel):
    def __init__(self, hg, n_chans=21, output_strat='gauss', preact='softmax', reg='none',
...
def _build_resnet_pose_model(base, dilate=0, truncate=0, output_strat='dsnt', preact='softmax',
    ...
    model = ResNetHumanPoseModel(
        resnet, n_chans=21, dilate=dilate, truncate=truncate, output_strat=output_strat,
...
def _build_hg_model(base, stacks=2, blocks=1, output_strat='gauss', preact='softmax',
    ...
    model = HourglassHumanPoseModel(hg, n_chans=21, output_strat=output_strat, preact=preact,
